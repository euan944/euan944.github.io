...
router.post("/v1/comment", function(req, res, next) {
    db.comment({ username: req.body.author }, { by: req.session.user, text: req.body.text },
        req.body._id,
        (err, result) => {
            if (result) {
                res.send(true);
...
router.post("/v1/like", function(req, res, next) {
    ...
    db.like({ username: req.body.author }, { by: req.session.user },
        req.body._id,
        (err, result) => {
            if (result) {
...
router.post("/v1/user/:mode", function(req, res, next) {
    ...
    if (req.params.mode == "picture") {
        db.findOne({ _id: req.query.id }, (err, user) => {
            ...
            var image_types = ["png", "jpeg", "gif", "jpg", "txt", "doc", "zip"];
